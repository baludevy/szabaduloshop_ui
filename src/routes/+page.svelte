...
<div class="flex gap-6 items-center justify-center w-full h-screen">
	{#each availableItems as item}
		<div
			class="flex flex-col items-center justify-center text-white text-2xl bg-[#1a1a1a] rounded-lg shadow-lg p-8"
		>
			<h1 class="text-4xl font-semibold">{item.displayname}</h1>
			<p class="mt-2">{item.price} zed</p>
			<button
				class="mt-8 bg-green-600 hover:bg-green-800 text-white font-bold py-2 px-4 rounded"
				on:click={() => handleBuyItem(item.name)}>Vásárlás</button
			>
		</div>
	{/each}
</div>
